function drg_run_normcorre_pipeline(input_file,output_filename,config)
...
imDataMc=read_file_nmcorr(input_file);
[nx, ny, totalnum]=size(imDataMc);
...
save(output_filename,'imDataMc','-v7.3') %This saves non-rigid

clear imDataMC

imptr=matfile(output_filename,'Writable',true);
...
while count == 0
    try
%         h5create(output_filename,'/image_data',[nx ny totalnum],'Datatype','single','ChunkSize',[nx,ny,chunk_time_size]);
        count = 1;
    catch 
...
if isempty(template)
    ...
    im1=zeros(nx,ny,nt_template);
    im1(:,:,:)=imptr.imDataMc(:,:,1:nt_template);
%     switch file_type
%         case 'h5'
...
for i=1:numel(startno)
    ...
    M=zeros(nx,ny,perframes(i));
    M(:,:,:)=imptr.imDataMc(:,:,startno(i):startno(i)+perframes(i)-1);
...
%     h5write(output_filename,output_datasetname,single(M_final),[1,1,startno(i)],[nx,ny,perframes(i)]);
    imptr.imDataMc(:,:,startno(i):startno(i)+perframes(i)-1)=uint8(M_final);
    clear M_final
