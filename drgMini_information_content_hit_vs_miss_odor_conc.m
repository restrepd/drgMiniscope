...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        for ii_ROI=1:no_neurons
            ...
            min_op_activity=min([this_Hit_op_activity(this_Hit_op_activity_n>0) this_Miss_op_activity(this_Miss_op_activity_n>0)]);
            ...
            for ii_op=1:11
                if (this_Hit_op_activity_n(ii_op)>0)
                    bar(ii_op,(this_Hit_op_activity(ii_op)-min_op_activity)/(max_op_activity-min_op_activity),'EdgeColor','none','FaceColor',[204/255 121/255 167/255])
                else
            ...
            for ii_op=1:11
                if (this_Miss_op_activity_n(ii_op)>0)
                    bar(ii_op,(this_Miss_op_activity(ii_op)-min_op_activity)/(max_op_activity-min_op_activity),'EdgeColor','none','FaceColor',[0/255 114/255 178/255])
                else
                    bar(ii_op,0.05,'EdgeColor','none','FaceColor',[150/255 150/255 150/255])
                end
            end
            ...
            sgt_legend=['dFF map file  No ' num2str(fileNo) ' ROI No ' num2str(ii_ROI) ' SSI op hit= ' num2str(ssi_all_op_info_hit(ii_ROI_all))...
            ...
            if ssi_all_op_info_hit(ii_ROI_all)>=3
                oc_ii=oc_ii+1;
                oc_dFF_norm_map_hit(oc_ii,:)=(this_Hit_op_activity-min_op_activity)/(max_op_activity-min_op_activity);
                oc_dFF_norm_map_hit_n(oc_ii,:)=this_Hit_op_activity_n;
                oc_dFF_norm_map_miss(oc_ii,:)=(this_Miss_op_activity-min_op_activity)/(max_op_activity-min_op_activity);
...
%Now plot the response of the odor concentraiton cells
% oc_dFF_norm_map_hit=zeros(sum(ssi_all_op_info_hit>=3),11);
% oc_dFF_norm_map_miss=zeros(sum(ssi_all_op_info_hit>=3),11);
% oc_dFF_norm_map_fileNo=zeros(1,sum(ssi_all_op_info_hit>=3));
% oc_dFF_norm_map_iiROI=zeros(1,sum(ssi_all_op_info_hit>=3));
...
for ii_oc=1:oc_ii
    ...
    this_oc_dFF_norm_map_hit(1,oc_dFF_norm_map_hit_n(ii_oc,:)==0)=-0.1;
    ...
    this_oc_dFF_norm_map_miss(1,oc_dFF_norm_map_miss_n(ii_oc,:)==0)=-0.1;
