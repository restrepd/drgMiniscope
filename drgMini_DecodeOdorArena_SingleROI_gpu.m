function handles_outXY=drgMini_DecodeOdorArena_SingleROI_gpu(handles_choices)
...
if exist('handles_choices')==0
    ...
    bins_before=0; %How many bins of neural data prior to the output are used for decoding, 4
...
for ii_sub=1:no_neurons
    ...
    X_dFF_trimmed=zeros(size(X_dFF,1),1);
    X_dFF_trimmed(:,:)=X_dFF(:,ii_sub);
    ...
    handles_outXY.ii_sub(ii_sub).x_predicted=x_predicted;
    handles_outXY.ii_sub(ii_sub).y_predicted=y_predicted;
    ...
    handles_outXY.ii_sub(ii_sub).x_predicted_conv=x_predicted_conv;
    handles_outXY.ii_sub(ii_sub).y_predicted_conv=y_predicted_conv;

    fprintf(1,['Done with ROI ' num2str(ii_sub) '\n'])
end
...
for ii_sub=1:no_neurons
    ...
    handles_outXY.ii_sub(ii_sub).x_predicted_sh=x_predicted;
    handles_outXY.ii_sub(ii_sub).y_predicted_sh=y_predicted;
    ...
    handles_outXY.ii_sub(ii_sub).x_predicted_conv_sh=x_predicted_conv;
    handles_outXY.ii_sub(ii_sub).y_predicted_conv_sh=y_predicted_conv;

    fprintf(1,['Done with ROI ' num2str(ii_sub) '\n'])
end
fprintf(1,['Elapsed time for all ROIs shuffled ' num2str((toc-start_toc)/(60*60)) ' hrs\n\n'])
...
save([handles_choices.save_path arena_file(1:end-4) handles_choices.save_tag '_singleROI.mat'],'handles_outXY','handles_choices','-v7.3')
