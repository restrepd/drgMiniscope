function handles_out=drgMini_dFFpredictionv3(handles_choices)
...
if exist('handles_choices')==0
    ...
    handles_choices.prctile_thr=80;

    handles_choices.binary_dFF=1;
...
if handles_choices.binary_dFF==1
    save_tag=[handles_choices.save_tag 'b' num2str(handles_choices.prctile_thr)];
else
    save_tag=handles_choices.save_tag;
end
...
if resume_processing==1
    load([this_path arena_file(1:end-4) '_' save_tag '_' num2str(handles_choices.algo) ...
...
if handles_choices.binary_dFF==1
    ...
    kernel_size = 2*ceil(3*sigma) + 1; % Size of the kernel
    ...
    for ii_neuron=1:no_neurons
        pffft=1;
        this_dFF=zeros(size(neural_data,1),1);
        this_binary_dFF=zeros(size(neural_data,1),1);
        this_dFF=neural_data(:,ii_neuron);
        dFF_th=prctile(this_dFF(this_dFF>0),handles_choices.prctile_thr);
        this_binary_dFF(this_dFF>=dFF_th)=1;
        this_conv_binary_dFF = conv(this_binary_dFF, gaussian_kernel, 'same');
        neural_data(:,ii_neuron)=this_conv_binary_dFF;
    end
end
    ...
    if isempty(per_ROI(this_ROI).results)
        ...
        while thisROI_done==0
            try
                ...
                if handles_choices.binary_dFF==1
                    this_dFF=zeros(sum(training_range_template),1);
                    this_dFF=neural_data(logical(training_range_template),this_ROI);
                    sum_dFF=sum(this_dFF);
                else
                    sum_dFF=1;
                end
                ...
                if sum_dFF~=0
                    ...
                    per_ROI(this_ROI).processed=1;
                else
                    fprintf(1,['Empty binary dFF\n\n'])
                    per_ROI(this_ROI).processed=0;
                end
                thisROI_done=1;
        ...
        % save([this_path arena_file(1:end-4) '_' save_tag '_' num2str(handles_choices.algo) ...
        ...
        if sum_dFF~=0
            ...
            per_ROI_sh(this_ROI).processed=1;
        else
            per_ROI_sh(this_ROI).processed=0;
        end
...
for ii_ROI=handles_choices.process_these_ROIs
    if per_ROI_sh(ii_ROI).processed==1
...
for this_ROI=handles_choices.process_these_ROIs
    if per_ROI(this_ROI).processed==1
