%drgMini_multi_ROI_information_content
close all
clear all

is_sphgpu=1; %0=Mac, 1=sphgpu, 2=Alpine
handles.is_sphgpu=is_sphgpu;
is_greedy=2; %1 uses the greedy function, 2 uses the best function
handles.is_greedy=is_greedy;
overwrite=1; %If 0 do not overwrite
...
if is_greedy==1
    outputFile='multi_ROI_info_content_greedy.mat';
else
        ...
        switch is_greedy
            case 1
                ...
                if overwrite==1
                    ...
                    proceed=0;
                    if fileNo>length(handles_out2.greedy.file)
                        proceed=1;
                    end
                end
                if proceed==1
                    start_toc=toc;
                    fprintf(1,['Starting greedy file No ' num2str(fileNo) ' \n\n'] )
                    if is_gpu==1
                        [handles_out2.greedy.file(fileNo).selected_neurons,...
                            handles_out2.greedy.file(fileNo).best_mi] =...
                            greedy_mi_selection_gpu(neural_data, binary_odor_plume, max_neurons);
                    else
                        [handles_out2.greedy.file(fileNo).selected_neurons,...
                            handles_out2.greedy.file(fileNo).best_mi] =...
                            greedy_mi_selection_cpu(neural_data, binary_odor_plume, max_neurons);
                    end
                    fprintf(1,['Duration (mins) ' num2str((toc-start_toc)/60) '\n\n'] )
                    fprintf(1,['Done with file No ' num2str(fileNo) ' n bits= '...
                        num2str(length(handles_out2.greedy.file(fileNo).selected_neurons(end))) ' best mi= '...
                        num2str(handles_out2.greedy.file(fileNo).best_mi(end)) '\n\n'] )
                    handles_out2.max_neurons=max_neurons;
...
% imp_all_info_ii_ROI=[];
imp_all_info_n_bits=[];
imp_all_info_ii_bits=[];
imp_all_info_op_bin=[];
imp_all_info_op_bin_sh=[];
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        for n_bits=all_n_bits
            these_ii_bits=find(n_bits==all_n_bits);
            ...
            for importance=1:2 %least and most
                ...
                for jj_ROI=1:length(these_ROIs)
                    all_these_dFF.ROI(jj_ROI).dFF=[];
                    all_these_dFF_reversed.ROI(jj_ROI).dFF=[];
                end
                ...
                for trNo=1:trials.odor_trNo
                    ...
                    for ii_t=1:length(these_x)
                        ...
                        for jj_ROI=1:length(these_ROIs)
                            ...
                            these_dFF=trials.trial(trNo).XdFFtest(:,ii_ROI);
                            if ii_t==1
                                all_these_dFF.ROI(jj_ROI).dFF=[all_these_dFF.ROI(jj_ROI).dFF; these_dFF];
                            end
                ...
                imp_all_info_n_bits(imp_all_info_ii)=n_bits;
                imp_all_info_ii_bits(imp_all_info_ii)=these_ii_bits;
                ...
                rng(ii_ROI)
                these_rnd=rand(1,2*n_shuffle_SI*n_bits);
                ...
                for ii_sh=1:n_shuffle_SI
                    ...
                    for jj_ROI=1:length(these_ROIs)
                        ...
                        ii_rnd=ii_rnd+1;
                        offset_ii=floor(these_rnd(ii_rnd)*length(all_these_dFF.ROI(jj_ROI).dFF));
                        these_all_these_dFF_reversed=zeros(1,length(all_these_dFF.ROI(jj_ROI).dFF));
                        these_all_these_dFF=all_these_dFF.ROI(jj_ROI).dFF;
                        for ii_trl=1:length(all_these_dFF)
                            ...
                            these_all_these_dFF_reversed(1,length(all_these_dFF)-ii_trl+1)=these_all_these_dFF(this_ii_trl);
                        end
                        all_these_dFF_reversed.ROI(jj_ROI).dFF=these_all_these_dFF_reversed;
                    end
                    ...
                    for ii_t=1:length(these_all_these_dFF_reversed)
                        ...
                        for jj_ROI=1:length(these_ROIs)
                            these_all_these_dFF_reversed=all_these_dFF_reversed.ROI(jj_ROI).dFF;
                            this_bin_dFF=(these_all_these_dFF_reversed(ii_t)>0)+1;
                            cum_bindFF(jj_ROI,this_bin_dFF)=cum_bindFF(jj_ROI,this_bin_dFF)+1;
                        end
                    ...
                    imp_all_info_op_bin_sh(imp_all_info_ii,ii_sh)=0;
                    ...
                    for ii_op=1:2
                        for ii_bits=1:n_bits
                            for ii_bin_dFF=1:2
                                if p_op_bindFF_bin(ii_bits,ii_bin_dFF,ii_op)~=0
                                    imp_all_info_op_bin_sh(imp_all_info_ii,ii_sh)=imp_all_info_op_bin_sh(imp_all_info_ii,ii_sh)+p_op_bindFF_bin(ii_bits,ii_bin_dFF,ii_op)*...
                                        log2(p_op_bindFF_bin(ii_bits,ii_bin_dFF,ii_op)/(p_op_bin(ii_op)*p_bindFF(ii_bits,ii_bin_dFF)));
                                end
                            end
                        end
            ...
            fprintf(1,['Done with file No ' num2str(fileNo) ' n bits ' num2str(n_bits) '\n\n'] )
        end
    end
end


%Plot mutual information between bindFF and op for most important
figureNo=figureNo+1;
try
    close(figureNo)
catch
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        these_info_op_bin=[];
        for ii=1:length(all_n_bits)
            these_info_op_bin(ii)=imp_all_info_op_bin((imp_all_info_fileNo==fileNo)&(imp_all_info_least_most==importance)&(imp_all_info_ii_bits==ii));
        end
...
xticks([1:length(all_n_bits)])
xticklabels({'1','5','10','20','All'})

xlabel('Number of ROIs')
ylabel('Mutual Information')
title('Mutual Information op and binary dFF including most important')


%Plot mutual information between bindFF and op for least important
figureNo=figureNo+1;
try
    close(figureNo)
catch
...
xlabel('Number of ROIs')
ylabel('Mutual Information')
title('Mutual Information op and binary dFF including least important')

%Below are the functions used by this script
function [selected_neurons,all_best_mi_combined] = greedy_mi_selection_cpu(neural_data, odor_plume, max_neurons)
    % Inputs:
    ...
    if max_neurons>num_neurons
        max_neurons=num_neurons;
    end
    selected_neurons = []; % Initialize empty set of selected neurons
    remaining_neurons = 1:num_neurons; % Indices of all neurons
    mi_values = zeros(1, num_neurons); % Store mutual information values for each neuron

    
    best_mi = -inf;
    all_best_mi_combined=zeros(1,max_neurons);
    %Now search for a new ROI and stop when the mi does not increase
    for k = 1:max_neurons
        
        best_neuron = -1;

        % Iterate through remaining neurons to find the one that maximizes MI
        for i = remaining_neurons
            current_subset = [selected_neurons, i]; % Add current neuron to the subset
            combined_activity = neural_data(:, current_subset); % Combine selected neuron activities
            thisROI_activity = neural_data(:, i); % Combine selected neuron activities

            % Compute mutual information between combined activity and odor plume

            mi = drg_compute_mutual_information_cpu(thisROI_activity, odor_plume);
            mi_combined = drg_compute_mutual_information_cpu(combined_activity, odor_plume);

            if mi_combined > best_mi
                best_mi = mi;
                best_mi_combined=mi_combined;
                best_neuron = i;
            end
        end

        if best_neuron == -1
            fprintf('Step %d: No more neurons selected because adding more ROIs does not yield an increase in MI\n');
            break
        else
            % Update selected and remaining neurons
            selected_neurons = [selected_neurons, best_neuron];
            remaining_neurons(remaining_neurons == best_neuron) = [];

            fprintf('Step %d: Selected Neuron %d with MI %.4f, combined MI %.4f\n', k, best_neuron, best_mi,best_mi_combined);
        end
        all_best_mi_combined(k)=best_mi_combined;
    end
end
...
%Below are the functions used by this script
function [selected_neurons,all_best_mi_combined] = greedy_mi_selection_gpu(neural_data, odor_plume, max_neurons)
    % Inputs:
    % neural_data: matrix of size [num_samples x num_neurons], where each column represents a neuron's activity.
    % odor_plume: vector of size [num_samples x 1], representing the odor plume signal.
    % max_neurons: maximum number of neurons to select.
    ...
    for k = 1:max_neurons
        ...
        for i = remaining_neurons
            ...
            mi = drg_compute_mutual_information_gpu(thisROI_activity, odor_plume);
...
function mi = drg_compute_mutual_information(data, target)
% Compute mutual information between data (matrix) and target (vector)
% This function assumes you have an external toolbox or custom function for MI computation.
%
% Replace this with your preferred method for computing MI
% data is bindFF(time_points,ROIs)
% target is bin_op(time_points,1)
...
for ii_t=1:length(target)
    cum_op_bin(target(ii_t)+1)=cum_op_bin(target(ii_t)+1)+1;
    for jj_ROI=1:n_bits
        this_bin_dFF=data(ii_t,jj_ROI)+1;
        cum_bindFF(jj_ROI,this_bin_dFF)=cum_bindFF(jj_ROI,this_bin_dFF)+1;
        cum_op_bindFF_bin(jj_ROI,this_bin_dFF,target(ii_t)+1)=cum_op_bindFF_bin(jj_ROI,this_bin_dFF,target(ii_t)+1)+1;
    end
end
...
function mi = compute_mutual_information(data, target)
    % Compute mutual information between data (matrix) and target (vector)
    % This function assumes you have an external toolbox or custom function for MI computation.
    %
    % Replace this with your preferred method for computing MI.
    
    % Example using MATLAB's built-in histogram-based estimation:
    joint_histogram = histcounts2(data(:), target(:), 'Normalization', 'probability');
    joint_entropy = -sum(joint_histogram(:) .* log2(joint_histogram(:) + eps));
    
    marginal_hist_data = histcounts(data(:), 'Normalization', 'probability');
    marginal_entropy_data = -sum(marginal_hist_data .* log2(marginal_hist_data + eps));
    
    marginal_hist_target = histcounts(target(:), 'Normalization', 'probability');
    marginal_entropy_target = -sum(marginal_hist_target .* log2(marginal_hist_target + eps));
    
    mi = marginal_entropy_data + marginal_entropy_target - joint_entropy;
end
