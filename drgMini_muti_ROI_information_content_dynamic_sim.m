%drgMini_multi_ROI_information_content_dynamic_sim
close all
clear all
...
all_n_bits=[1 1 1]; %Number of bits (ROIs) for most and least important prediction ROIs
                            %And for best
alpha=0.475*ones(1,200); %Used for simuations
handles_out2.alpha=alpha;
        ...
        for ii_alpha=1:length(alpha)
            ...
            for ii_ROI=1:no_neurons
                ...
                for trNo=1:trials.odor_trNo
                    ...
                    these_rand=rand(1,length(these_x));
                    these_bin_dFF=zeros(length(these_x),1);
                    for ii_t=1:length(these_x)
                        ...
                        if ii_t<=ii_t_odor_on
                            ...
                            these_bin_op(ii_t)=0;
                            if these_rand(ii_t)>alpha(ii_alpha)
                                these_bin_dFF(ii_t)=0;
                            else
                                these_bin_dFF(ii_t)=1;
                            end
                        else
            ...
            handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).hit_mi=drg_compute_mutual_information_cpu(hit_neural_data,hit_binary_odor_plume);
            fprintf(1,['For file No ' num2str(fileNo) ' alpha= ' num2str(alpha(ii_alpha)) ' mi for hits= '...
                num2str(handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).hit_mi) '\n\n'] )
            ...
            handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).miss_mi=drg_compute_mutual_information_cpu(miss_neural_data,miss_binary_odor_plume);
            fprintf(1,['For file No ' num2str(fileNo) ' alpha= ' num2str(alpha(ii_alpha)) ' mi for misses= '...
                num2str(handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).miss_mi) '\n\n'] )
            ...
            for ii_sh=1:n_shuffle_SI
                these_hit_neural_data=zeros(sum(hits),1);
                these_miss_neural_data=zeros(sum(~hits),1);

                these_hit_neural_data(:,1)=shuffled_neural_data.sh(ii_sh).ROI(ii_ROI).neural_data(logical(hits));
                these_miss_neural_data(:,1)=shuffled_neural_data.sh(ii_sh).ROI(ii_ROI).neural_data(~logical(hits));

                this_mi = drg_compute_mutual_information_cpu(these_hit_neural_data, binary_odor_plume(logical(hits)));
                handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).sh_hit_mi(ii_sh)=this_mi;

                this_mi = drg_compute_mutual_information_cpu(these_miss_neural_data, binary_odor_plume(~logical(hits)));
                handles_out2.hit_miss.file(fileNo).ii_alpha(ii_alpha).sh_miss_mi(ii_sh)=this_mi;
            end
