...
switch is_sphgpu
    case 0
        ...
        %Trained with all trials and mult=1 taking on account when mouse detects the odor
        %0 bins before run with drgMini_DecodeDynamicOdorConcSingleROIs.m
        %Only the current bin
        ...
        choiceOdorConcFileName{1}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02032026.m';
         ...
         %0 bins before re-run with drgMini_DecodeDynamicOdorConcSingleROIsv2.m 
        save_PathConc{2}='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConcSingle_02232026/';
        choiceOdorConcFileName{2}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02232026.m';

        %The files below were run with drgMini_DecodeDynamicOdorConcSingleROIs.m
        %and are predictions with one previous time bin determined by before_bins
        %For example if bins_before=2 the prediction was performed with the second
        %previous time min
        %5 bins before
        save_PathConc{3}='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConcSingle_02142026/';
        choiceOdorConcFileName{3}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02142026.m';
        % 
        %2 bins before 
        save_PathConc{4}='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConcSingle_02152026/';
        choiceOdorConcFileName{4}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02152026.m';
        % 
        %10 bins before, flawed code for before_bins, do not use 
        save_PathConc{5}='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConcSingle_02162026/';
        choiceOdorConcFileName{5}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02162026.m';
        % 
        % %1 bin before 
        save_PathConc{6}='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConcSingle_02182026/';
        choiceOdorConcFileName{6}='drgDynamicOdorConcChoices_Fabio_SingleROIs_02182026.m';

        single_time_bin_files=[1 3:6];
        ...
        fileID = fopen([choiceBatchPathName 'decode_XYandconc_SingleROIPrev_stats.txt'],'w');

        saveFile='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/SingleROI_bins_before_conc_analysis.mat';
...
% handles.bins_before_conc=[0 0 0 0 0 0 0 0 1 2 4];
...
for ConcFileNo=1:length(save_PathConc)
    for fileNo=1:length(handles_conc.arena_file)
        if sum(handles_conc.group(fileNo)==these_groups)>0
            %Load conc data
            ...
            % %load the ouptut file
            % conc_fileNo=1;
            load([save_PathConc{ConcFileNo} arena_file(1:end-4) handles_conc.save_tag{ii_run} '_singleROI.mat'])
            handles_anal_conc.ConcFile(ConcFileNo).handles_choices=handles_choices;
            bins_before_conc(ConcFileNo)=sum(handles_choices.bins_before);
            trials=handles_out.trials;
            ...
            for ii_sub=1:handles_out.no_neurons
                ...
                if ~isempty(op_all_trials)
                    ...
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials=this_r2ER;
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).p_r2ER_all=this_P_r2ER;
                else
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials=NaN;
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).p_r2ER_all=NaN;
                end
                ...
                if ~isempty(op_all_trials)
                    % r2ER_all_trials(fileNo)=drgMini_r2ER(op_all_trials,op_decod_all_trials);
                    [this_r2ER,this_P_r2ER]=drgMini_r2ER_bootstrap(op_all_trials,op_decod_all_trials,10000,is_hat);
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials_sh=this_r2ER;
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).p_r2ER_all_sh=this_P_r2ER;
                else
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials_sh=NaN;
                    handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).p_r2ER_all_sh=NaN;
                end
...
save(saveFile,'handles_anal_conc','-v7.3')
...
these_r2ERs_conc=[];
these_means_r2ERs_conc=[];
bar_offsets=[];

these_r2ER_sh_conc=[];

binb_index=1:length(bins_before_conc);
to_sort=[binb_index; bins_before_conc]';
sorted=sortrows(to_sort,2);
binb_index_sorted=sorted(:,1);
bins_before_conc_sorted=sorted(:,2);

for ConcFileNo=binb_index_sorted'
    %First get the r2ERs for each number of ROIs
    these_r2ERs=[];

    for fileNo=1:length(handles_conc.arena_file)
        if sum(handles_conc.group(fileNo)==these_groups)>0
            for ii_sub=1:length(handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub)
                these_r2ERs=[these_r2ERs handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials];
            end
        end
    end

    these_r2ERs_conc.ConcFile(ConcFileNo).r2ER=these_r2ERs;
    these_means_r2ERs_conc=[these_means_r2ERs_conc mean(these_r2ERs)];
    ...
    for fileNo=1:length(handles_conc.arena_file)
        if sum(handles_conc.group(fileNo)==these_groups)>0
            for ii_sub=1:length(handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub)
                these_r2ER_sh=[these_r2ER_sh handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials_sh];
            end
        end
    end

    these_r2ERs_sh_conc.ConcFile(ConcFileNo).r2ER=these_r2ER_sh;
    ...
    glm_r2ER.before(glm_r2ER_ii+1:glm_r2ER_ii+length(these_r2ERs))=bins_before_conc(ConcFileNo)*ones(1,length(these_r2ERs));
    glm_r2ER_ii=glm_r2ER_ii+length(these_r2ERs);

    id_r2ER_ii=id_r2ER_ii+1;
    input_r2ER_data(id_r2ER_ii).data=these_r2ERs;
    input_r2ER_data(id_r2ER_ii).description=['Original b' num2str(bins_before_conc(ConcFileNo))];
    ...
    id_r2ER_ii=id_r2ER_ii+1;
    input_r2ER_data(id_r2ER_ii).data=these_r2ERs;
    input_r2ER_data(id_r2ER_ii).description=['Shuffled b' num2str(bins_before_conc(ConcFileNo))];


end

plot(bar_offsets,these_means_r2ERs_conc,'-o','Color',[0.7 0.7 0.7],'LineWidth',2,'MarkerFaceColor',[0.7 0.7 0.7])

xlim([-0.5 bar_offset-0.5])
xticks(bar_offsets+0.5);           % place ticks at 0, 2, 5, 10 [web:7]
xticklabels(string(bins_before_conc_sorted));  % use those numbers as labels [web:1]
  % tick labels from your cell array
xlabel('Bins before')
ylabel('r2ER for odor concentration')
title('Decoding with different bins before')

%Perform the glm  for prediction of odor all, hit, miss, shuffled
fprintf(1, ['\nglm for Fig ' num2str(figureNo) ' for r2ER vs before bin number\n']);
% fprintf(1, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);
fprintf(fileID, ['\nglm for Fig ' num2str(figureNo) ' for r2ER vs before bin number number\n']);
% fprintf(fileID, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);



tbl = table(glm_r2ER.data',glm_r2ER.shuffled',glm_r2ER.before',...
    'VariableNames',{'r2ES','shuffled','bins_before_conc'});
mdl = fitglm(tbl,'r2ES~shuffled+bins_before_conc'...
    ,'CategoricalVars',[2,3])
...
fprintf(1, ['\n\nRanksum or t-test p values for r2ER vs number of bins before\n'])
fprintf(fileID, ['\n\nRanksum or t-test p values for r2ER vs number of bins before\n']);
...
for ConcFileNo=1:length(handles_anal_conc.ConcFile)
    ...
    for fileNo=1:length(handles_conc.arena_file)
        if sum(handles_conc.group(fileNo)==these_groups)>0
            for ii_sub=1:length(handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub)
                these_r2ERs_conc=[these_r2ERs_conc handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials];
    ...
    for fileNo=1:length(handles_conc.arena_file)
        if sum(handles_conc.group(fileNo)==these_groups)>0
            for ii_sub=1:length(handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub)
                these_r2ER_sh_conc=[these_r2ER_sh_conc handles_anal_conc.ConcFile(ConcFileNo).file(fileNo).ii_sub(ii_sub).r2ER_all_trials_sh];
            end
        end
    end
    ...
    handles_anal_conc.ConcFile(ConcFileNo).these_r2ERs_conc=these_r2ERs_conc;
    handles_anal_conc.ConcFile(ConcFileNo).these_ROIs_conc= these_ROIs_conc;
    handles_anal_conc.ConcFile(ConcFileNo).these_files_conc=these_files_conc;
    handles_anal_conc.ConcFile(ConcFileNo).these_r2ER_sh_conc=these_r2ER_sh_conc;
    handles_anal_conc.ConcFile(ConcFileNo).SD_these_r2ERs_sh_conc=SD_these_r2ERs_sh_conc;
    handles_anal_conc.ConcFile(ConcFileNo).mean_these_r2ERs_sh_conc=mean_these_r2ERs_sh_conc;
    handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc=these_SD_significant_r2ERs_conc;
    handles_anal_conc.ConcFile(ConcFileNo).these_sig_r2ERs_conc=handles_anal_conc.ConcFile(ConcFileNo).these_r2ERs_conc(...
        logical(handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc));
...
% Define Okabe-Ito color palette (RGB triplets)
okabe_ito_colors = [...
...
n_bars = 5;  % number of cumulative histograms
hold on

for i=1:length(handles_anal_conc.ConcFile)
    % Compute ECDF using your custom function
    [f_aic, x_aic] = drg_ecdf(handles_anal_conc.ConcFile(i).these_sig_r2ERs_conc);

    % Select color cyclically if more bars than colors
    color_idx = mod(i-1, size(okabe_ito_colors,1)) + 1;
    this_color = okabe_ito_colors(color_idx, :);

    % Plot ECDF
    plot(x_aic, f_aic, 'Color', this_color, 'LineWidth', 3);
end
...
% 
% [f_aic,x_aic] = drg_ecdf(handles_anal_conc.ConcFile(1).these_sig_r2ERs_conc);
% plot(x_aic,f_aic,'Color',[0.7 0.7 0.7],'LineWidth',3)
% 
% [f_aic,x_aic] = drg_ecdf(handles_anal_conc.ConcFile(2).these_sig_r2ERs_conc);
% plot(x_aic,f_aic,'Color',[0 0.6 0.5],'LineWidth',3)
% 
% 
% [f_aic,x_aic] = drg_ecdf(handles_anal_conc.ConcFile(3).these_sig_r2ERs_conc);
% plot(x_aic,f_aic,'Color',[0.9 0.6 0],'LineWidth',3)

ylabel('Fraction')
xlabel('r2ER')
title('Cumulative histogram r2ER for the different number of bins before')
...
for ConcFileNo=binb_index_sorted'
    ...
    these_r2ERs=handles_anal_conc.ConcFile(ConcFileNo).these_sig_r2ERs_conc;
    ...
    id_r2ER_ii=id_r2ER_ii+1;
    input_r2ER_data(id_r2ER_ii).data=these_r2ERs;
    input_r2ER_data(id_r2ER_ii).description=['No bins before ' num2str(bins_before_conc(ConcFileNo))];
...
xlim([-0.5 bar_offset-0.5])
xticks(bar_offsets);           % place ticks at 0, 2, 5, 10 [web:7]
xticklabels(string(bins_before_conc_sorted));  % use those numbers as labels [web:1]
  % tick labels from your cell array
xlabel('Bins before')
ylabel('Significant r2ER OC')
title('Decoding with different bins before')

%Perform the glm  for prediction of odor all, hit, miss, shuffled
fprintf(1, ['\nglm for Fig ' num2str(figureNo) ' for significant r2ER vs before bin number\n']);
% fprintf(1, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);
fprintf(fileID, ['\nglm for Fig ' num2str(figureNo) ' for significant r2ER vs before bin number number\n']);
% fprintf(fileID, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);



tbl = table(glm_r2ER.data',glm_r2ER.before',...
    'VariableNames',{'r2ES','bins_before'});
mdl = fitglm(tbl,'r2ES~bins_before'...
    ,'CategoricalVars',[2])
...
%Do the ranksum/t-test
fprintf(1, ['\n\nRanksum or t-test p values for significant r2ER vs number of bins before\n'])
fprintf(fileID, ['\n\nRanksum or t-test p values for significant r2ER vs number of bins before\n']);

 
[output_data] = drgMutiRanksumorTtest(input_r2ER_data, fileID,0);

%Now show the r2ERs for different before bins vs 0
for ConcFileNo=2:length(binb_index_sorted)
    figureNo = figureNo + 1;
    try
        close(figureNo)
    catch
    ...
    these_r2ERs_current=handles_anal_conc.ConcFile(1).these_r2ERs_conc(...
        logical(handles_anal_conc.ConcFile(1).these_SD_significant_r2ERs_conc)...
        &logical(handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc));

    these_r2ERs_before=handles_anal_conc.ConcFile(ConcFileNo).these_r2ERs_conc(...
        logical(handles_anal_conc.ConcFile(1).these_SD_significant_r2ERs_conc)...
        &logical(handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc));

    max_r2ER=max([max(these_r2ERs_before) max(these_r2ERs_current)]);
    plot([0 max_r2ER],[0 max_r2ER],'-','LineWidth',2,'Color',[0.7 0.7 0.7])

    plot(these_r2ERs_current,these_r2ERs_before,'.k')
    
    xlim([0 max_r2ER])
    ylim([0 max_r2ER])
    xlabel('r2ER no previous data')
    ylabel(['r2ER ' num2str(bins_before_conc(ConcFileNo)) ' bins before'])

end

%Now for the single time bin files show which previous time bin gives the
%best r2ER

%Generate a vector of ROIs taht have significant r2ERs in any of the
%ConcFiles
any_sig=logical(zeros(1,length(handles_anal_conc.ConcFile(1).these_SD_significant_r2ERs_conc)));
for ii_file=1:length(single_time_bin_files)
    ConcFileNo=single_time_bin_files(ii_file);
    any_sig=any_sig|logical(handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc);
end

%Now get the data for each ROI
norm_r2ER_time_course=zeros(sum(any_sig),length(single_time_bin_files));
r2ER_time_course=zeros(sum(any_sig),length(single_time_bin_files));
peaks_r2ER_time_course=zeros(sum(any_sig),1);
ii_these_ROIs=0;
for iiROI=1:length(handles_anal_conc.ConcFile(ConcFileNo).these_SD_significant_r2ERs_conc)
    if any_sig(iiROI)==1
        ii_these_ROIs=ii_these_ROIs+1;
        these_r2ER_time_course=zeros(1,length(single_time_bin_files));
        for ii_file=1:length(single_time_bin_files)
            ConcFileNo=single_time_bin_files(ii_file);
            these_r2ER_time_course(1,ii_file)=handles_anal_conc.ConcFile(ConcFileNo).these_r2ERs_conc(iiROI);
        end
        [M,I]=max(these_r2ER_time_course);
        peaks_r2ER_time_course(ii_these_ROIs)=I;
        norm_r2ER_time_course(ii_these_ROIs,:)=these_r2ER_time_course/M;
        r2ER_time_course(ii_these_ROIs,:)=these_r2ER_time_course;
    end
end

to_sort=[-peaks_r2ER_time_course norm_r2ER_time_course];
sorted_rows=sortrows(to_sort);

sorted_norm_r2ER_time_course=sorted_rows(:,2:end);


%Plot the pseudocolor
figureNo=figureNo+1;
try
    close(figureNo)
catch
...
hFig = figure(figureNo);
set(hFig, 'units','normalized','position',[.1 .1 .3 .75])


these_bins_before_conc_sorted=bins_before_conc_sorted(single_time_bin_files);
drg_pcolor(repmat([1:length(these_bins_before_conc_sorted)]',1,ii_these_ROIs)',repmat([1:ii_these_ROIs]',1,length(these_bins_before_conc_sorted)),sorted_norm_r2ER_time_course)
colormap(fire)
clim([0 1.1]);
shading flat

xticks([1:length(these_bins_before_conc_sorted)]+0.5);           % place ticks at 0, 2, 5, 10 [web:7]
xticklabels(string(these_bins_before_conc_sorted));

xlabel('Time bins before current')
ylabel('ROI')

title('Normalized r2ER OC')
...
% %Plot cumulative histograms for the peak r2ERs
% figureNo=figureNo+1;
% try
%     close(figureNo)
% catch
...
% hFig = figure(figureNo);
% set(hFig, 'units','normalized','position',[.1 .1 .3 .75])
% %Now do a cumulative histo of the peak r2ERs 
% % norm_r2ER_time_course(ii_these_ROIs,:)=these_r2ER_time_course/M;
% %         r2ER_time_course(ii_these_ROIs,:)=these_r2ER_time_course;
% 
% 
% hold on
% 
% for i=1:length(these_bins_before_conc_sorted)
%     these_norm_r2ERs=zeros(1,size(norm_r2ER_time_course,1));
%     these_norm_r2ERs(1,:)=norm_r2ER_time_course(:,i);
%     these_r2ERs=zeros(1,size(norm_r2ER_time_course,1));
%     these_r2ERs(1,:)=r2ER_time_course(:,i);
%     % Compute ECDF using your custom function
%     [f_aic, x_aic] = drg_ecdf(these_r2ERs(these_norm_r2ERs==1));
% 
%     % Select color cyclically if more bars than colors
%     color_idx = mod(i-1, size(okabe_ito_colors,1)) + 1;
%     this_color = okabe_ito_colors(color_idx, :);
...
%Plot bar graphs for the peak r2ERs
figureNo = figureNo + 1;
try
    close(figureNo)
catch
...
for i=1:length(these_bins_before_conc_sorted)
    ...
    these_r2ERs=zeros(1,size(norm_r2ER_time_course,1));
    these_r2ERs(1,:)=r2ER_time_course(:,i);
    these_r2ERs=these_r2ERs(these_norm_r2ERs==1);
    ...
    glm_r2ER.data(glm_r2ER_ii+1:glm_r2ER_ii+length(these_r2ERs))=these_r2ERs;
    glm_r2ER.before(glm_r2ER_ii+1:glm_r2ER_ii+length(these_r2ERs))=these_bins_before_conc_sorted(i)*ones(1,length(these_r2ERs));
    glm_r2ER_ii=glm_r2ER_ii+length(these_r2ERs);

    id_r2ER_ii=id_r2ER_ii+1;
    input_r2ER_data(id_r2ER_ii).data=these_r2ERs;
    input_r2ER_data(id_r2ER_ii).description=['No bins before ' num2str(these_bins_before_conc_sorted(i))];
...
xlim([-0.5 bar_offset-0.5])
xticks(bar_offsets);           % place ticks at 0, 2, 5, 10 [web:7]
xticklabels(string(these_bins_before_conc_sorted));  % use those numbers as labels [web:1]
  % tick labels from your cell array
xlabel('Bins before')
ylabel('r2ER OC')
title('r2ERs for peak decoding with different bins before')

%Perform the glm  for prediction of odor all, hit, miss, shuffled
fprintf(1, ['\nglm for Fig ' num2str(figureNo) ' for significant peak r2ER vs before bin number\n']);
% fprintf(1, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);
fprintf(fileID, ['\nglm for Fig ' num2str(figureNo) ' for significant peak r2ER vs before bin number number\n']);
% fprintf(fileID, ['\n1 Hit, 2 All, 3 Miss, 4 Shuffled\n']);
...
%Do the ranksum/t-test
fprintf(1, ['\n\nRanksum or t-test p values for significant peak r2ER vs number of bins before\n'])
fprintf(fileID, ['\n\nRanksum or t-test p values for significant peak r2ER vs number of bins before\n']);
