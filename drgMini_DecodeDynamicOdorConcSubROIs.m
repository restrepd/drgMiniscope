function handles_out=drgMini_DecodeDynamicOdorConcSubROIs(handles_choices)
...
for ROIsubset=1:length(handles_choices.no_sub_ROIs)
    ...
    handles_out.ROIsubset(ROIsubset).idx=idx;
    for ii_sub=1:handles_choices.no_runs__per_sub_ROI(ROIsubset)
        ...
        handles_out.ROIsubset(ROIsubset).ii_sub(ii_sub).op_predicted=op_predicted;
        ...
        handles_out.ROIsubset(ROIsubset).ii_sub(ii_sub).op_predicted_conv=op_predicted_conv;
...
for ROIsubset=1:length(handles_choices.no_sub_ROIs)
    ...
    for ii_sub=1:handles_choices.no_runs__per_sub_ROI(ROIsubset)
        ...
        handles_out.ROIsubset(ROIsubset).ii_sub(ii_sub).op_predicted_sh=op_predicted;
        ...
        handles_out.ROIsubset(ROIsubset).ii_sub(ii_sub).op_predicted_conv_sh=op_predicted_conv;

        fprintf(1,['Done with shuffled ROI susbet ' num2str(ROIsubset) ' run number ' num2str(ii_sub) '\n'])
    end

    fprintf(1,['Elapsed time for shuffled ROI susbet ' num2str(ROIsubset) ' ' num2str((toc-start_toc)/(60*60)) ' hrs\n\n'])
...
save([handles_choices.save_path arena_file(1:end-4) handles_choices.save_tag '_subROI.mat'],'handles_out','handles_choices','-v7.3')
