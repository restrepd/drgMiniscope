...
%Decode using neural network repeated fits
n_repeats=5;

Y1_valid_n=zeros(size(Xvalid,1),n_repeats);
Y2_valid_n=zeros(size(Xvalid,1),n_repeats);
Y1_valid=zeros(size(Xvalid,1),1);
Y2_valid=zeros(size(Xvalid,1),1);
for ii=1:n_repeats
    MdlY1 = fitrnet(Xtrain,Ytrain(:,1));
    MdlY2 = fitrnet(Xtrain,Ytrain(:,2));

    Y1_valid_n(:,ii)=predict(MdlY1,Xvalid);
    Y2_valid_n(:,ii)=predict(MdlY2,Xvalid);
end

Y1_valid(:,1)=mean(Y1_valid_n,2);
Y2_valid(:,1)=mean(Y2_valid_n,2);
...
hold on
plot(Yvalid(y1v_start:y1v_end,1),Y1_valid_conv(y1v_start:y1v_end,1),'-b')
xlabel('Actual x')
ylabel('Decoded x')
title('x for neural network convolved')
...
hold on
plot(Yvalid(y1v_start:y1v_end,2),Y2_valid_conv(y1v_start:y1v_end,1),'.b')
xlabel('Actual y')
ylabel('Decoded y')
title('y for neural network convolved')


% %Decode using optimized neural network
% %This was not any better!
% MdlY1 = fitrnet(Xtrain,Ytrain(:,1),'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('UseParallel',true));
% MdlY2 = fitrnet(Xtrain,Ytrain(:,2),'OptimizeHyperparameters','auto','HyperparameterOptimizationOptions',struct('UseParallel',true));
% 
% Y1_valida=predict(MdlY1,Xvalid);
% Y2_valida=predict(MdlY2,Xvalid);
% 
% fprintf(1, 'R2 nn: %d %d\n',drgGetR2(Yvalid(:,1),Y1_valida),drgGetR2(Yvalid(:,2),Y2_valida));
% R1=corrcoef(Yvalid(:,1),Y1_valida);
% R2=corrcoef(Yvalid(:,2),Y2_valida);
% fprintf(1, 'Correlation coefficient nn optimized: %d %d\n\n',R1(1,2),R2(1,2));
% 
% figNo=figNo+1;
% try
%     close(figNo)
...
% 
% hold on
% plot(Y1_valida(y1v_start:y1v_end,1),Y2_valida(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,1),Yvalid(y1v_start:y1v_end,2),'-b','LineWidth',1.5)
% title('xy for neural network optimized')
% 
% figNo=figNo+1;
% try
%     close(figNo)
...
% 
% hold on
% plot(Y1_valida(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,1),'-b','LineWidth',1.5)
% title('x for neural network optimized')
% 
% figNo=figNo+1;
% try
%     close(figNo)
...
% 
% hold on
% plot(Y2_valida(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,2),'-b','LineWidth',1.5)
% title('y for neural network optimized')
% 
% 
% 
% no_conv_points=11;
...
% conv_win_gauss=conv_win_gauss/sum(conv_win_gauss);
% 
% Y1_valida_conv=conv(Y1_valida,conv_win_gauss,'same');
% Y2_valida_conv=conv(Y2_valida,conv_win_gauss,'same');
% 
% %Now limit the x and y to max and min
% minY1=min(Yvalid(:,1));
% Y1_valida_conv(Y1_valida_conv<minY1)=minY1;
% maxY1=max(Yvalid(:,1));
% Y1_valida_conv(Y1_valida_conv>maxY1)=maxY1;
% 
% minY2=min(Yvalid(:,2));
% Y2_valida_conv(Y2_valida_conv<minY2)=minY2;
% maxY2=max(Yvalid(:,2));
% Y2_valida_conv(Y2_valida_conv>maxY2)=maxY2;
% 
% 
% fprintf(1, 'R2 nn conv: %d %d\n',drgGetR2(Yvalid(:,1),Y1_valida_conv),drgGetR2(Yvalid(:,2),Y2_valida_conv));
% R1=corrcoef(Yvalid(:,1),Y1_valida_conv);
% R2=corrcoef(Yvalid(:,2),Y2_valida_conv);
% fprintf(1, 'Correlation coefficient nn optimized conv %d %d\n\n',R1(1,2),R2(1,2));
% 
% 
% figNo=figNo+1;
% try
...
% 
% hold on
% plot(Y1_valida_conv(y1v_start:y1v_end,1),Y2_valida_conv(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,1),Yvalid(y1v_start:y1v_end,2),'-b','LineWidth',1.5)
% title('xy for neural network optimized convolved')
% 
% figNo=figNo+1;
% try
%     close(figNo)
...
% 
% hold on
% plot(Y1_valida_conv(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,1),'-b','LineWidth',1.5)
% title('x for neural network optimized convolved')
% 
% figNo=figNo+1;
% try
%     close(figNo)
...
% 
% hold on
% plot(Y2_valida_conv(y1v_start:y1v_end,1),'-r','LineWidth',1.5)
% plot(Yvalid(y1v_start:y1v_end,2),'-b','LineWidth',1.5)
% title('y for neural network optimized convolved')
% pffft=1;
