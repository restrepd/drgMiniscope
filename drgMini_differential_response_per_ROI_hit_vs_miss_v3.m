%drgMini_differential_response_perROI_hit_vs_miss_v3
close all
clear all
...
save_path_file='/data2/SFTP/glm_div_end.mat';
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        for ii_ROI=1:no_neurons
            ...
            %These are aligned to the reward
            hit_dFF_end=[];
            ii_hit_end=0;
            miss_dFF_end=[];
            ii_miss_end=0;
            ...
            for trNo=1:trials.odor_trNo
                ...
                this_ii_odor_end=trials.odor_ii_end(trNo);

                if ((this_ii_odor_end-ii_for_time_bins/2) > 0) && (((this_ii_odor_end+ii_for_time_bins/2) < no_time_bins))

                    these_dFF=zeros(1,length(odor_aligned_time_bins));
                    these_dFF(1,:)=neural_data(this_ii_odor_end-ii_for_time_bins/2:this_ii_odor_end+ii_for_time_bins/2,ii_ROI);

                    switch trials.odor_trial_type(trNo)
                        case 1
                            %Lane 1 hits vermillion
                            ii_hit_end=ii_hit_end+1;
                            hit_dFF_end(ii_hit_end,1:length(odor_aligned_time_bins))=these_dFF;

               
                        case 2
                            %Lane 1 miss orange
                            ii_miss_end=ii_miss_end+1;
                            miss_dFF_end(ii_miss_end,1:length(odor_aligned_time_bins))=these_dFF;
            ...
            handles_out2.file(fileNo).ROI(ii_ROI).hit_dFF_end=mean(hit_dFF_end,1);
            handles_out2.file(fileNo).ROI(ii_ROI).miss_dFF_end=mean(miss_dFF_end,1);
