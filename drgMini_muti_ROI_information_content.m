...
all_n_bits=[1 5 10 20 2000]; %Number of bits (ROIs) for most and least important prediction ROIs
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        max_neurons=[1 3 5 10 20 100000];
        for ii_neurons=max_neurons
                [greedy_output.file(fileNo).selected_neurons,greedy_output.file(fileNo).best_mi] = best_mi_selection(neural_data, binary_odor_plume, ii_neurons);
                fprintf(1,['Done with file No ' num2str(fileNo) ' n bits= ' num2str(length(greedy_output.file(fileNo).selected_neurons)) ' best mi= '...
                    num2str(greedy_output.file(fileNo).best_mi) '\n\n'] )
        end
        pfft=1;
...
function [selected_neurons,best_mi_combined] = greedy_mi_selection(neural_data, odor_plume, max_neurons)
    % Inputs:
    ...
    for k = 1:max_neurons
        ...
        for i = remaining_neurons
            ...
            mi = drg_compute_mutual_information(thisROI_activity, odor_plume);
            mi_combined = drg_compute_mutual_information(combined_activity, odor_plume);
...
function [selected_neurons,best_mi] = best_mi_selection(neural_data, odor_plume, n_subset)
    % Inputs:
    % neural_data: matrix of size [num_samples x num_neurons], where each column represents a neuron's activity.
    % odor_plume: vector of size [num_samples x 1], representing the odor plume signal.
    % max_neurons: maximum number of neurons to select.
...
function mi = drg_compute_mutual_information_gpu(data, target)
    ...
    cum_op_bin = accumarray(target_indices, 1, [2, 1], @sum, 0, true);
    
    for jj_ROI = 1:n_bits
        cum_bindFF(jj_ROI, :) = accumarray(data_indices(:, jj_ROI), 1, [2, 1], @sum, 0, true);
        for ii_op = 1:2
            cum_op_bindFF_bin(jj_ROI, :, ii_op) = accumarray([data_indices(:, jj_ROI), target_indices], 1, [2, 1], @sum, 0, true);
        end
    end
    ...
    joint_prob = p_op_bindFF_bin;
    marginal_prob = p_op_bin .* p_bindFF;
    
    log_ratio = log2(joint_prob ./ marginal_prob);
    log_ratio(isinf(log_ratio) | isnan(log_ratio)) = 0;  % Handle division by zero
    
    mi = sum(joint_prob .* log_ratio, 'all');
