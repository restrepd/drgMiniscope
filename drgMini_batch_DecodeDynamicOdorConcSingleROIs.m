%drgMini_batch_DecodeDynamicOdorConcSubROIs
%This does batch decoding for subsets of ROIs
[choiceFileName,choiceBatchPathName] = uigetfile({'drgDynamicOdorConcChoices_*.m'},'Select the .m file with all the choices for analysis');


fprintf(1, ['\ndrgMini_batch_DecodeDynamicOdorConc run for ' choiceFileName '\n\n']);
...
if all_files_present==1
    ...
    for fileNo=first_file:handles.no_files
        if sum(handles.process_these_groups==handles.group(fileNo))>0
            ...
            handles_choices.air_flow_speed=handles.air_flow_speed;

            % handles_choices.no_sub_ROIs=[1 5 10 20 40 500];
            % handles_choices.no_runs__per_sub_ROI=[40 30 30 30 30 1];
            ...
            for ii_run=first_run:length(handles.weber_fechner)
                ...
                if process_data==1
                    ...
                    % while is_done==0
                    %     try
                            handles_out=drgMini_DecodeDynamicOdorConcSingleROIsv2(handles_choices);
                            is_done=1;
                    %     catch
