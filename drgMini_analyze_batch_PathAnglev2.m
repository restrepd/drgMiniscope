%drgMini_analyze_batch_PathAnglev2
close all
clear all
...
%Please note that the spouts are 7 and 5 cm away from the wall
y_lane1=430;
y_lane4=70; %Please note that this is correct and was measured
hit_radius=100;
    ...
    % if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
      if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        for trNo=1:trials.odor_trNo
            ...
            if fileNo==20
                %These are here to understand the mouse's path
                hFig = figure(2*trNo-1);
                set(hFig, 'units','normalized','position',[.1 .1 .3 .3])

                %Plot the mouse's path
                plot(these_x,these_y,'-k')
                hold on

                %Start point
                plot(these_x(1),these_y(1),'ob')

                %Hit radii
                lane_y=[y_lane1 y_lane4];
                for ii_lane=1:2
                    % Parameters
                    cx = 0;           % center x
                    cy = lane_y(ii_lane);          % center y
                    r  = hit_radius;         % radius

                    % Angle from -90° to +90° so the arc faces towards x > 0
                    theta = linspace(-pi/2, pi/2, 200);

                    % Parametric equation of the semicircle
                    x = cx + r*cos(theta);
                    y = cy + r*sin(theta);
                ...
                ylim([0 500])
                axis square
                yticks([y_lane4 y_lane1])
                yticklabels({'lane 4','lane 1'})
                switch trials.odor_trial_type(trNo)
                    case 1
                        %Lane 1 hits
                        title(['trial ' num2str(trNo) ' lane 1 HIT'])
                    case 2
                        %Lane 1 miss
                        title(['trial ' num2str(trNo) ' lane 1 MISS'])
                    case 3
                        %Lane 4 hits
                        title(['trial ' num2str(trNo) ' lane 4 HIT'])
                    case 4
                        %Lane 4 miss
                        title(['trial ' num2str(trNo) ' lane 4 MISS'])
                end

                %Now plot the speed
                hFig = figure(2*trNo);
                set(hFig, 'units','normalized','position',[.1 .1 .3 .15])

                these_speeds=zeros(1,length(these_x)-1);
                these_times=zeros(1,length(these_x)-1);
                these_lane1=zeros(1,length(these_x)-1);
                these_lane4=zeros(1,length(these_x)-1);


                for ii_t=1:length(these_x)-1
                    delta_d=sqrt((these_x(ii_t+1)-these_x(ii_t))^2+(these_y(ii_t+1)-these_y(ii_t))^2);
                    these_speeds(ii_t)=delta_d/handles_XY.dt;
                    these_times(ii_t)=ii_t*handles_XY.dt + 0.5*handles_XY.dt;

                    %Is this within lane 1 spout range
                    delta_d_lane_1=sqrt(((these_x(ii_t+1)+these_x(ii_t))/2)^2+(((these_y(ii_t+1)+these_y(ii_t))/2)-y_lane1)^2);
                    if delta_d_lane_1<=hit_radius
                        these_lane1(ii_t)=1;
                    end

                    %Is this within lane 4 spout range
                    delta_d_lane_4=sqrt(((these_x(ii_t+1)+these_x(ii_t))/2)^2+(((these_y(ii_t+1)+these_y(ii_t))/2)-y_lane4)^2);
                    if delta_d_lane_4<=hit_radius
                        these_lane4(ii_t)=1;
                    end
                end

                hold on
                for ii_t=1:length(these_times)
                    if (these_lane1(ii_t)==0)&(these_lane4(ii_t)==0)
                        plot(these_times(ii_t),these_speeds(ii_t)/10,'.k')
                    end
                    if (these_lane1(ii_t)==1)
                        plot(these_times(ii_t),these_speeds(ii_t)/10,'.r')
                    end
                    if (these_lane4(ii_t)==1)
                        plot(these_times(ii_t),these_speeds(ii_t)/10,'.b')
                    end
                end
                ...
                switch trials.odor_trial_type(trNo)
                    case 1
                        %Lane 1 hits
                        title(['Speed for trial ' num2str(trNo) ' lane 1 HIT'])
                    case 2
                        %Lane 1 miss
                        title(['Speed for trial ' num2str(trNo) ' lane 1 MISS'])
                    case 3
                        %Lane 4 hits
                        title(['Speed for trial ' num2str(trNo) ' lane 4 HIT'])
                    case 4
                        %Lane 4 miss
                        title(['Speed for trial ' num2str(trNo) ' lane 4 MISS'])
                end
                pffft=1;
                % figNo=2*trNo;
            end
        ...
        handles_out_dt_spout.file(fileNo).mean_session_dt_spout_hits=mean(this_file_dt_spout_hits);
        handles_out_dt_spout.file(fileNo).mean_session_dt_spout_miss=mean(this_file_dt_spout_miss);
...
for ii=1:length(per_session_dt_spout_hits)
    plot([bar_offset-1 bar_offset],[per_session_dt_spout_hits(ii),per_session_dt_spout_miss(ii)],...
        'o-','Color',[0.7 0.7 0.7],'MarkerEdgeColor',[0.7 0.7 0.7],'MarkerFaceColor',[0.7 0.7 0.7],'markerSize',4,'LineWidth',1)
end
...
[h,p]=ttest2(dt_spout_hits,dt_spout_miss);
 ...
 [h,p,ci,stats] = vartest2(dt_spout_hits,dt_spout_miss);

 fprintf(1,['\np value for variance of time near spout ' num2str(p) '\n'])
...
save([save_outPathAngle save_outFileAngle],'handles_out_dt_spout')
