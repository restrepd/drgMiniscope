...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        lane_trial_cells=[];
        these_p_values_per_ROI=zeros(no_neurons,1);
        ...
        for ii_ROI=1:no_neurons
            ...
            hit_dFF_turn=zeros(sum(trials.hit1)+sum(trials.hit4),sum((time_bins>=trimmed_time_range(1))&(time_bins<=trimmed_time_range(2))));
            miss_dFF_turn=zeros(sum(trials.miss1)+sum(trials.miss4),sum((time_bins>=trimmed_time_range(1))&(time_bins<=trimmed_time_range(2))));
            ...
            for trNo=1:trials.odor_trNo
                ...
                switch trials.odor_trial_type(trNo)
                    case 1
                        ...
                        ii_hit=ii_hit+1;
                        hit_dFF(ii_hit,1:length(time_bins))=min(these_dFF);
                        hit_dFF(ii_hit,delta_below_zero_ii-all_ii_turns(trNo)+1:delta_below_zero_ii+(all_ii_ends(trNo)-all_ii_turns(trNo)))=these_dFF;
                        hit_dFF_turn(ii_hit,:)=min(these_dFF);
                        for ii_tur=1:length(turn_time_bins)
                            if sum(these_time_bins==turn_time_bins(ii_tur))
                                hit_dFF_turn(ii_hit,ii_tur)=these_dFF(these_time_bins==turn_time_bins(ii_tur));
                            end
                        end
                    ...
                    case 2
                        %Lane 1 miss orange
                        ii_miss=ii_miss+1;
                        miss_dFF(ii_miss,1:length(time_bins))=min(these_dFF);
                        miss_dFF(ii_miss,delta_below_zero_ii-all_ii_turns(trNo)+1:delta_below_zero_ii+(all_ii_ends(trNo)-all_ii_turns(trNo)))=these_dFF;
                        miss_dFF_turn(ii_miss,:)=min(these_dFF);
                        for ii_tur=1:length(turn_time_bins)
                            if sum(these_time_bins==turn_time_bins(ii_tur))
                                miss_dFF_turn(ii_miss,ii_tur)=these_dFF(these_time_bins==turn_time_bins(ii_tur));
                            end
                        end
                        for ii_tr=1:length(trimmed_time_bins)
                            these_tr_time_bins=(these_time_bins>=(trimmed_time_range(1)+(ii_tr-1)*trimmed_dt))&(these_time_bins<(trimmed_time_range(1)+ii_tr*trimmed_dt));
            ...
            handles_out2.file(fileNo).ROI(ii_ROI).hit_dFF_turn=mean(hit_dFF_turn,1);
            handles_out2.file(fileNo).ROI(ii_ROI).miss_dFF_turn=mean(miss_dFF_turn,1);
            ...
            y_gap=2;
            hit_miss=zeros(size(hit_dFF,1)+size(miss_dFF,1)+y_gap,delta_below_zero_ii+delta_above_zero_ii);
            ...
            drg_pcolor(repmat(time_bins,length(y_trials),1),repmat(y_trials,length(time_bins),1)',hit_miss)
            colormap(this_cmap)
            ...
            rectangle('Position', [time_bins(1), size(miss_dFF,1)+1, 1.03*(time_bins(end)-time_bins(1)), y_gap], ... % [x, y, width, height]
            ...
            p_values(ii_all_pvalues,1)=coefTest(mdl,[0 1 0]);


            these_p_values_per_ROI(ii_ROI,1)=coefTest(mdl,[0 1 0]);
...
fprintf(1, ['\n\nPercent of cells showing divergent responses between hit and miss in ' num2str(handles_out2.no_files)  ' sessions ' ...
    num2str(100*handles_out2.ii_comp(ii_comp).no_significant/handles_out2.ii_comp(ii_comp).no_neurons) '\n'])
    ...
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(files_included(fileNo)==1)
        ...
        for ii_ROI=1:no_neurons
            if these_significant(ii_ROI)==1
                all_div_hit=[all_div_hit; handles_out2.file(fileNo).ROI(ii_ROI).hit_dFF_turn];
                all_div_miss=[all_div_miss; handles_out2.file(fileNo).ROI(ii_ROI).miss_dFF_turn];
            end
        end
