%drgMini_display_batch_DecodeCorrXYConcSubROIs
close all
clear all
...
switch is_sphgpu
    case 0
        ...
        fileID = fopen([choiceBatchPathName 'decode_XYandconc_SubROI_stats.txt'],'w');
...
for ROIsubset=1:length(handles_choices.no_sub_ROIs)
    ...
    if ROIsubset<length(handles_choices.no_sub_ROIs)
        ...
        ROIsubset_labels{ROIsubset}='All sh';
    end
...
fprintf('For sig r2ERx vs r2ER odor concentration\n')
fprintf('Extra-SS F-test (linear vs quadratic): F(%.0f,%.0f)=%.2f,p=%.3g\n', ...
        df1, df2, F, p_F);
...
fprintf('For sig r2ERy vs r2ER odor concentration\n')
fprintf('Extra-SS F-test (linear vs quadratic): F(%.0f,%.0f)=%.2f,p=%.3g\n', ...
        df1, df2, F, p_F);
