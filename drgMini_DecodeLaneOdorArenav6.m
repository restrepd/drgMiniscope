function handles_out=drgMini_DecodeLaneOdorArenav6(handles_choices2)
...
if exist('handles_choices2')==0
    ...
    which_ROIs=4; %1 Use all ROIs, 2 Use place cells, 3 use odor cells, 4 use ROIs in
    %handles_choices2.process_these_ROIs
    handles_choices2.which_ROIs=which_ROIs;

    handles_choices2.process_these_ROIs=17; %This is only used if which_ROIs=4
    ...
    which_shuffle=handles_choices2.which_shuffle;
...
for ii_shuffled=1:n_shuffle
    ...
    % parfor trNo=1:trials.odor_trNo
        ...
        XdFFtrain=X_dFF(this_training_range,:);
        if which_shuffle==2
            rp_ii=randperm(length(XdFFtrain(:)));
            newXdFFtrain=XdFFtrain;
            for ii=1:length(XdFFtrain(:))
                newXdFFtrain(ii)=XdFFtrain(rp_ii(ii));
            end
            XdFFtrain=newXdFFtrain;
            newXdFFtrain=[];
        end
        % Xvalid=X_dFF(ii_valid_range(1):ii_valid_range(2),:);
        XdFFtest=X_dFF(logical(this_test_range),:);
...
for trNo=1:trials.odor_trNo
    ...
    for ii_rp=1:no_repeats
        if this_display_start<1
            this_accuracy=[accuracy(1,ii_rp)*ones(-this_display_start+1,1); accuracy(1:this_display_end,ii_rp)];
        else
            this_accuracy=accuracy(this_display_start:this_display_end,ii_rp);
        end
        accuracy_all_trials=[accuracy_all_trials this_accuracy];
    end
...
%Save data
handles_out.start_alligned.accuracy_all_trials=accuracy_all_trials;
handles_out.start_alligned.time_span=time_span;
handles_out.start_alligned.accuracy_sh_all_trials=accuracy_sh_all_trials;
...
%Save data
handles_out.end_alligned.accuracy_all_trials=accuracy_all_trials;
handles_out.end_alligned.time_span=time_span;
handles_out.end_alligned.accuracy_sh_all_trials=accuracy_sh_all_trials;
