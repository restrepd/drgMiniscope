%drgMini_analyze_batch_DecodeCorrXYConcv2
close all
clear all
...
for fileNo=1:length(handles_conc.arena_file)
    ...
    if isfile([save_PathConc arena_file(1:end-4) handles_conc.save_tag{ii_run} '.mat'])
        ...
        if ~isempty(op_all_trials)
            ...
            [r2ER,this_P_r2ER_all_trials]=drgMini_r2ER_bootstrap(op_all_trials,op_decod_all_trials,10000);
            r2ER_all_trials_pre(fileNo)=R1(1,2);
            P_r2ER_all_trials_pre(fileNo)=this_P_r2ER_all_trials;
        else
            R1_all_trials_pre(fileNo)=NaN;
            r2ER_all_trials_pre(fileNo)=NaN;
        end
    else
        P_rho_all_trials_pre(fileNo)=1;
        P_r2ER_all_trials_pre(fileNo)=1;
    end
end
  
pFDRrho = drsFDRpval(P_rho_all_trials_pre);
pFDRr2ER = drsFDRpval(P_r2ER_all_trials_pre);

p_bonf_rho = 0.05/length(P_rho_all_trials_pre);
p_bonf_r2ER = 0.05/length(P_r2ER_all_trials_pre);
...
%Calculate the percent of sessions with p<p_thr
...
for grNo=these_groups
    ...
    for fileNo=1:length(handles_conc.arena_file)
        ...
        if (handles_conc.group(fileNo)==grNo)&(fraction_other_angle(fileNo)<thr_froa)
            ...
            if (P_r2ER_all_trials_pre(fileNo)<=thr_rho)
...
fprintf(1, ['\nPercent sessions with r2ER p<p_thr for odor trials ' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n'])
fprintf(fileID, ['\nPercent sessions with r2ER p<p_thr for odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n']);
...
fprintf(1, ['\nPercent sessions with R1 p<p_thr for odor trials ' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n'])
fprintf(fileID, ['\nPercent sessions with R1 p<p_thr for odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n']);
...
fprintf(1, ['\nPercent sessions for r2ER with p<p_thr for no odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n'])
fprintf(fileID, ['\nPercent sessions for r2ER with p<p_thr for no odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n']);
...
fprintf(1, ['\nPercent sessions for R1 with p<p_thr for no odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n'])
fprintf(fileID, ['\nPercent sessions for R1 with p<p_thr for no odor trials' num2str(100*sessions_p_less/total_sessions) ' , total sessions ' num2str(total_sessions)  '\n']);
...
for grNo=these_groups
    ...
    for fileNo=1:length(handles_conc.arena_file)
        ...
        if (handles_conc.group(fileNo)==grNo)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
            %load the ouptut file
            load([save_PathConc arena_file(1:end-4) handles_conc.save_tag{ii_run} '.mat'])
            these_R1s=[these_R1s handles_out.R1.all_hits];
        end
...
for grNo=1:1
    ...
    for fileNo=1:length(handles_conc.arena_file)
        if (sum(handles_conc.group(fileNo)==these_new_groups.gr(grNo).groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
            ...
            %load the ouptut file
            load([save_PathConc arena_file(1:end-4) handles_conc.save_tag{ii_run} '.mat'])
            R1_conc=[R1_conc handles_out.R1.all_hits];
            R1_per_mouse.group(grNo).mouse(handles_conc.mouse(fileNo)).R1_conc=...
                [R1_per_mouse.group(grNo).mouse(handles_conc.mouse(fileNo)).R1_conc handles_out.R1.all_hits];
    ...
    p = polyfit(R1_conc, R1_XY, 1);        % Fit a first-degree polynomial (a line)
    R1_XY_fit = polyval(p, R1_conc);        % Evaluate the fitted line at your x data

    plot(R1_conc, R1_XY_fit, '-k','LineWidth',2)              % Plot original data as circles
  
end

% text(0.5,0.5,'Both spouts','Color','k')
% text(0.5,0.4,'One spout','Color',[0.7 0.7 0.7])
title('R1 for hits conc vs. XY')
xlabel('R1 conc')
ylabel('R1 XY')

[this_R1,this_P]=corrcoef(R1_conc,R1_XY);
...
% 
% 
% title(['R1, correlation between decoded odor concentration and average odor plume'])
% ylabel('R1')
% ylim([-0.2 0.65])
% xlim([-1 17])
...
% 
% plot(percent_correct,these_R1s,'ok','MarkerEdgeColor','none','MarkerFaceColor','k')
% 
% 
% % Fit a first-degree polynomial (straight line)
% p = polyfit(percent_correct, these_R1s, 1);
% 
...
% 
% [rho, pval] = corr(percent_correct(:), these_R1s(:), 'Type', 'Spearman')
% 
% title(['R1 hits vs. percent correct behavior ' group_labels{grNo}])
% ylabel('R1')
% xlabel('Percent correct')
% 
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
        ...
        if ~isempty(op_all_trials)
            ...
            R2_rect_all_trials(ii_for_corr)=drgMini_rectifyR2(this_R2);
            % r2ER_all_trials(ii_for_corr)=drgMini_r2ER(op_all_trials,op_decod_all_trials);
            Var_per_point_all(ii_for_corr)=sum((op_all_trials-mean(op_all_trials)).^2)/length(op_all_trials);
        else
            R2_rect_all_trials(ii_for_corr)=NaN;
            r2ER_all_trials(ii_for_corr)=NaN;
        end

        if ~isempty(op_all_hits)
            this_R2=1-sum((op_decod_all_hits-op_all_hits).^2)/(sum((op_all_hits-mean(op_all_hits)).^2));
            R2_rect_hits(ii_for_corr)=drgMini_rectifyR2(this_R2);
            % r2ER_hits(ii_for_corr)=drgMini_r2ER(op_all_hits,op_decod_all_hits);
            Var_per_point_hits(ii_for_corr)=(sum((op_all_hits-mean(op_all_hits)).^2))/length(op_all_hits);
        else
            R2_rect_hits(ii_for_corr)=NaN;
        end

        if ~isempty(op_all_miss)
            this_R2=1-sum((op_decod_all_miss-op_all_miss).^2)/(sum((op_all_miss-mean(op_all_miss)).^2));
            R2_rect_miss(ii_for_corr)=drgMini_rectifyR2(this_R2);
            % r2ER_miss(ii_for_corr)=drgMini_r2ER(op_all_miss,op_decod_all_miss);
        ...
        if ~isempty(op_predicted_sh_conv)
            ...
            R2_rect_all_trials_sh(ii_for_corr)=mean(these_R2s);
            % r2ER_all_trials_sh(ii_for_corr)=mean(these_r2ERs);
            Var_per_point_all_sh(ii_for_corr)=mean(these_Vars);
        else
            R2_rect_all_trials_sh(ii_for_corr)=NaN;
        end
...
for fileNo=1:length(handles_conc.arena_file)
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
        ...
        ii_for_corr_r2ER=ii_for_corr_r2ER+1;
        these_fileNos_rER2(ii_for_corr_r2ER)=fileNo;

        %Calculate R2, the fraction of variance explained
        if ~isempty(op_all_trials)
            r2ER_all_trials(ii_for_corr_r2ER)=drgMini_r2ER(op_all_trials,op_decod_all_trials);
        else
            r2ER_all_trials(ii_for_corr_r2ER)=NaN;
        end

        if ~isempty(op_all_hits)
            r2ER_hits(ii_for_corr_r2ER)=drgMini_r2ER(op_all_hits,op_decod_all_hits);
        else
            r2ER_hits(ii_for_corr_r2ER)=NaN;
        end

        if ~isempty(op_all_miss)
            r2ER_miss(ii_for_corr_r2ER)=drgMini_r2ER(op_all_miss,op_decod_all_miss);
        else
            r2ER_miss(ii_for_corr_r2ER)=NaN;
        end
        ...
        if ~isempty(op_predicted_sh_conv)
            ...
            r2ER_all_trials_sh(ii_for_corr_r2ER)=mean(these_r2ERs);
        else
            r2ER_all_trials_sh(ii_for_corr_r2ER)=NaN;
        end
...
% Convert Condition to categorical if it isn't already
Condition = categorical(glm_r1.trial_type');

% Create a table for the fitlm function
tbl = table(these_R1s, Condition, these_Variances, 'VariableNames',{'R1','Condition','Variance'});

% Fit the linear model (ANCOVA)
mdl = fitlm(tbl, 'R1 ~ Condition+Variance');
...
fprintf(1, ['\n\nAnalysis of covariance to determnine whether the Miss vs Hit difference is\n independent variance of the odor data\n\n'])
