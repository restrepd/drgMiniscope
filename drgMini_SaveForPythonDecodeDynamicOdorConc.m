function handles_out=drgMini_SaveForPythonDecodeDynamicOdorConc(handles_choices)
...
handles_outp=[]; %Output file for Python
handles_outp.xdFF=[];
handles_outp.bxdFF=[];
handles_outp.op=[];
handles_outp.pos_binned=[];
handles_outp.trials=trials;
...
for trNo=1:trials.odor_trNo
    ...
    this_test_range(op_predictedstart:op_predictedend)=1;
    ...
    these_dFF=X_dFF(logical(this_test_range),:);
    b_these_dFF=(these_dFF>0);  %Make binary for Naive Bayesian

    handles_outp.trial_start_ii(trNo)=size(handles_outp.xdFF,1)+1;

    handles_outp.xdFF=[handles_outp.xdFF; these_dFF];
    handles_outp.bxdFF=[handles_outp.bxdFF; b_these_dFF];
    handles_outp.op=[handles_outp.op; odor_plume_template(:,logical(this_test_range))'];
    handles_outp.pos_binned=[handles_outp.pos_binned pos_binned(logical(this_test_range),:)'];
    handles_outp.trial_end_ii(trNo)=size(handles_outp.xdFF,1);
...
%Plot the shifted odor plume
hFig = figure(figNo);
set(hFig, 'units','normalized','position',[.1 .1 .6 .3])

plot(handles_outp.op)
...
edges=[-11:0.05:-2];
histogram(handles_outp.op, edges)

counts = histcounts(handles_outp.op, edges);
[maxCount, idx] = max(counts);
binCenter = (edges(idx) + edges(idx+1)) / 2;

% histogram(handles_outp.op)

xlabel('log10(op)')
ylabel('Counts')

handles_outp.op(handles_outp.op<binCenter)=binCenter;
...
handles_outp.pos_binned=handles_outp.pos_binned';
% handles_outp.op=100*(handles_outp.op-min(handles_outp.op)); %This is needed for bayesian
handles_outp.x=[handles_outp.pos_binned(:,1)];
handles_outp.y=[handles_outp.pos_binned(:,2)];
save([this_path dFF_file(1:end-4) '_pyth2.mat'],'handles_outp','-v7.3')
pfft=1;
