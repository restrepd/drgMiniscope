function handles_out=drgMini_DecodeOdorPlume(handles_choices2)
...
if exist('handles_choices2')==0
    ...
    is_sphgpu=handles_choices2.is_sphgpu;
    handles_conc=handles_choices2.handles_conc;
    fileNo=handles_choices2.fileNo;
...
for trNo=1:trials.odor_trNo
    if (all_ii_turns(trNo)-(3/dt)>0)&(all_ii_turns(trNo)+(3/dt)<=size(trials.trial(trNo).XdFFtest,1))
        ...
        these_trimmed_x=zeros(length(trimmed_time),1);
        these_trimmed_y=zeros(length(trimmed_time),1);
        these_trimmed_x(:,:)=trials.trial(trNo).XYtest(all_ii_turns(trNo)-(3/dt):all_ii_turns(trNo)+(3/dt),1);
        these_trimmed_y(:,:)=trials.trial(trNo).XYtest(all_ii_turns(trNo)-(3/dt):all_ii_turns(trNo)+(3/dt),2);

        these_trimmed_lanes=zeros(1,length(trimmed_time));
        for ii_t=1:length(these_trimmed_x)
                    this_x_ii=ceil(these_trimmed_x(ii_t)/50);
                    if this_x_ii==11
                        this_x_ii=10;
                    end

                    this_y_ii=ceil(these_trimmed_y(ii_t)/48);
                    if this_y_ii==11
                        this_y_ii=10;
                    end
                    ...
                    if trials.lane_per_trial(trNo)==1
                        ...
                        switch handles_conc.group(fileNo)
                            case 1
                                %2 cm from the floor
                                cm_from_floor=2;
                                these_trimmed_lanes(ii_t)=odor_plume_patterns.cm_from_floor(cm_from_floor).binary_plumel1(this_x_ii,this_y_ii);
                            case 5
                                %1 cm from the floor
                                cm_from_floor=1;
                                these_trimmed_lanes(ii_t)=odor_plume_patterns.cm_from_floor(cm_from_floor).binary_plumel1(this_x_ii,this_y_ii); 
                        ...
                        switch handles_conc.group(fileNo)
                            case 1
                                %2 cm from the floor
                                cm_from_floor=2;
                                these_trimmed_lanes(ii_t)=odor_plume_patterns.cm_from_floor(cm_from_floor).binary_plumel4(this_x_ii,this_y_ii);
                            case 5
                                %1 cm from the floor
                                cm_from_floor=1;
                                these_trimmed_lanes(ii_t)=odor_plume_patterns.cm_from_floor(cm_from_floor).binary_plumel4(this_x_ii,this_y_ii);
