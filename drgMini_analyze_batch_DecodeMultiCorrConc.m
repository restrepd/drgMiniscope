...
switch is_sphgpu
    case 0
        ...
        % choiceOdorConcFileName='drgDynamicOdorConcChoices_Fabio_bin_all_0p1_10202025.m';

        %Trained with all trials and mult=0.5 taking on account when mouse detects the odor
        % save_PathConc='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConc0p510162025/';
        % choiceOdorConcFileName='drgDynamicOdorConcChoices_Fabio_bin_all_0p5_10162025.m';

        %Trained with all trials and mult=1 taking on account when mouse detects the odor
        % save_PathConc='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConc_1_10182025/';
        % choiceOdorConcFileName='drgDynamicOdorConcChoices_Fabio_bin_all_1_10182025.m';

        %Trained with all trials and mult=0.05 taking on account when mouse detects the odor
        % save_PathConc='/Users/restrepd/Documents/Projects/SFTP/Fabio_OdorArena_GoodData/DecodeDynOdorConc_0p05_10192025/';
        % choiceOdorConcFileName='drgDynamicOdorConcChoices_Fabio_bin_all_0p05_10192025.m';
...
% Fit a linear model
fitResult = fit(R1_conc', R1_XY', 'poly1'); % 'poly1' specifies a linear fit

% Extract slope and intercept
slope = fitResult.p1; % Slope
intercept = fitResult.p2; % Intercept

plot([-0.1 0.5],[-0.1*slope+intercept 0.5*slope+intercept],'-k','LineWidth',2)
...
for fileNo=1:length(handles_conc.arena_file)
    ...
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
        ...
        %load the ouptut file
        load([save_PathConc arena_file(1:end-4) handles_conc.save_tag{ii_run} '.mat'])
        these_R1s_odor=[these_R1s_odor handles_out.R1.all_trials];
    end
end
...
for fileNo=1:length(handles_conc.arena_file)
    ...
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
        ...
        %load the ouptut file
        load([save_PathConc arena_file(1:end-4) handles_conc.save_tag{ii_run} '.mat'])
        these_R1s_no_odor=[these_R1s_no_odor handles_out.R1.all_trials];
    end
end
...
title(['R1 for prediction of odor vs. mo odor'])
ylabel('R1')
ylim([-0.25 1])
xlim([-1 2])
    ...
    if (sum(handles_conc.group(fileNo)==these_groups)>0)&(fraction_other_angle(fileNo)<thr_froa)&(P_rho_all_trials_pre(fileNo)<=thr_rho)
        ...
        for trNo=1:trials.odor_trNo
            ...
            if ~isempty(ii_turn)
                ...
                if op_predictedstart_bt<=0
                    op_predictedstart_bt=1;
                end
